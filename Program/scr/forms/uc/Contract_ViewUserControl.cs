...
{
    public partial class Contract_ViewUserControl : UserControl
    {
        ...
        public Contract_ViewUserControl()
        {
            InitializeComponent();

            this.Load += Contract_ViewForm_Load;
            this.Disposed += Contract_ViewForm_Disposed;
            ...
            if (
                Core.ThisUser.AccessLevel == "HeadManager" ||
                Core.ThisUser.AccessLevel == "Admin"
                )
                this.Controls.Add(button_edit);

            if (Core.ThisUser.AccessLevel == "Admin") this.Controls.Add(button_remove);
        ...
        private void Contract_ViewForm_Disposed(object? sender, EventArgs e)
        {
            this.Load -= Contract_ViewForm_Load;
            this.Disposed -= Contract_ViewForm_Disposed;
            button_update.Click -= Button_update_Click;
        ...
        {
            new Contract_AddEditForm(DBT_Contract.GetById((int)dataGridView.CurrentCell.OwningRow.Cells[0].Value)).ShowDialog();
            UpdateTable();
        }
        ...
        private void Button_remove_Click(object? sender, EventArgs e)
        {
            ...
            int res = DBT_Contract.Remove((int)dataGridView.CurrentCell.OwningRow.Cells[0].Value);
        ...
        private void Contract_ViewForm_Load(object sender, EventArgs e) => UpdateTable();
        ...
        private void UpdateTable()
        {
            ...
            dataGridView.Columns.Add("ID_Contract", "№");
            dataGridView.Columns.Add("ID_Client", "Клиент");
            dataGridView.Columns.Add("ID_Tariff", "Тариф");
            dataGridView.Columns.Add("ID_Employee", "Сотрудник");
            dataGridView.Columns.Add("SigningDate", "Дата заключения");
                ...
                using (var query = connection.CreateCommand())
                {
                    ...
                    using (var reader = query.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            dataGridView.Rows[index].Cells[1].Value = DBT_Client.GetById(reader.GetInt32(1)).FullName;
                            dataGridView.Rows[index].Cells[2].Value = DBT_TariffPlan.GetById(reader.GetInt32(2)).Name;
                            dataGridView.Rows[index].Cells[3].Value = DBT_Employee.GetById(reader.GetInt32(3)).FullName;
